...
from metrics_m import dice_liver_3d, dice_lesion_3d
...
#  data folder
parser.add_argument('-data', type=str, default='/media/buyizhiyou/zly/liverdata/data/', help='test images')
parser.add_argument('-save_path', type=str, default='/media/buyizhiyou/zly/liverdata/result/HDUnet/hff')
#  other paras
parser.add_argument('-b', type=int, default=1)
parser.add_argument('-input_size', type=int, default=224)
#parser.add_argument('-model_weight', type=str, default='./model/weights-2d.64-0.04.hdf5')
#parser.add_argument('-model_weight', type=str, default='./model/weights.48-0.06.hdf5')
parser.add_argument('-model_weight', type=str, default='/media/buyizhiyou/zly/liverdata/result/HDUnet/3d/H_model/weights.30-0.05.hdf5')
...
def train_and_predict(args):
    ...
    model.compile(optimizer=sgd, loss=[weighted_crossentropy12], metrics=[dice_liver_3d, dice_lesion_3d])
    #model.load_weights(args.model_weight, by_name=True)
    model.load_weights(args.model_weight)
    #print (model.summary())

    #  liver tumor LITS
    trainidx = list(range(60))
    img_list = []
    tumor_list = []
    minindex_list = []
    maxindex_list = []
    ...
    for idx in range(60):
        ...
        tumor, tumor_header = load('/media/buyizhiyou/LITS/Training_Batch/segmentation-' + str(idx) + '.nii')
        img_list.append(img)
    ...
    model.fit_generator(generate_arrays_from_file(args.b, trainidx, img_list, tumor_list, tumorlines, liverlines,
                        ...
                        epochs= 600, verbose = 1, callbacks = [model_checkpoint], max_queue_size=10, workers=3, use_multiprocessing=True)
