    ...
    for cols in range(left_cols,right_cols+window_cols,window_cols):
        ...
        if cols > z - img_cols:
            ...
            for i in range(batch):
                ...
                score[left_deps:right_deps, left_rows:right_rows, z-img_cols+1:z-1, :] += patch_test_mask[i]
                score_num[left_deps:right_deps, left_rows:right_rows, z-img_cols+1:z-1, :] += 1
        else:
            '''
            if mini[1] > y - img_rows:
                left_rows = y - img_rows
            ...
            patch_test_mask = patch_test_mask[:,:,:,1:-1,:]
            for i in range(batch):
                score[left_deps:right_deps, left_rows:right_rows, cols+1:cols+img_cols-1, :] += patch_test_mask[i]
                score_num[left_deps:right_deps, left_rows:right_rows, cols+1:cols+img_cols-1, :] += 1
