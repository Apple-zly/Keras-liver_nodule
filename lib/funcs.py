    ...
    for cols in range(left_cols,right_cols+window_cols,window_cols):
        ...
        if cols > z - img_cols:
            if mini[1] > y - img_rows:
                patch_test = imgs_test[mini[0]:mini[0]+img_deps, y-img_rows:y, z-img_cols:z]
            else:
                patch_test = imgs_test[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows, z-img_cols:z]
            ...
            for i in range(batch):
                if mini[1] > y - img_rows:
                    ...
                    score[mini[0]:mini[0]+img_deps, y-img_rows:y, z-img_cols+1:z-1, :] += patch_test_mask[i]
                    score_num[mini[0]:mini[0]+img_deps, y-img_rows:y,  z-img_cols+1:z-1, :] += 1
                else:
                    score[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows, z-img_cols+1:z-1, :] += patch_test_mask[i]
                    score_num[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows,  z-img_cols+1:z-1, :] += 1
        else:
            if mini[1] > y - img_rows:
                patch_test = imgs_test[mini[0]:mini[0]+img_deps, y-img_rows:y, z-img_cols:z]
            else:
                patch_test = imgs_test[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows, cols:cols + img_cols]
            box_test[count, :, :, :, 0] = patch_test
            patch_test_mask = model.predict(box_test, batch_size=batch, verbose=0)
            patch_test_mask = K.softmax(patch_test_mask)
            patch_test_mask = K.eval(patch_test_mask)
            ...
            for i in range(batch):
                if mini[1] > y - img_rows:
                    score[mini[0]:mini[0]+img_deps, y-img_rows:y, cols+1:cols+img_cols-1, :] += patch_test_mask[i]
                    score_num[mini[0]:mini[0]+img_deps, y-img_rows:y, cols+1:cols+img_cols-1, :] += 1
                else:
                    score[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows, cols+1:cols+img_cols-1, :] += patch_test_mask[i]
                    score_num[mini[0]:mini[0]+img_deps, mini[1]:mini[1]+img_rows, cols+1:cols+img_cols-1, :] += 1
...
def predict_window_mulgpu(model,batch, imgs_test, img_deps, img_rows, img_cols, multiloss=False):
    ...
    for deps in range(0,x-img_deps+window_deps,window_deps):
        ...
        for rows in range(0, y-img_rows+window_rows, window_rows):
            ##slice too much, the memory is not enough
            if z > 700:
                left_cols = 300
            else:
                left_cols = 0
            for cols in range(left_cols,z-img_cols+window_cols,window_cols):
                if deps>x-img_deps:
                    deps = x-img_deps
